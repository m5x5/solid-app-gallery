// Capture Media Kraken (noeldemartin.github.io/media-kraken) onboarding +
// functionality, logging in with the test pod account.
// Flow: "where to store?" -> Use Solid -> WebID form -> CSS login -> collection.
...
const APP_ID = "urn:uuid:865a30b8-b57f-472d-b9cc-99f0a5c591e6"; // Media Kraken
const URL = "https://noeldemartin.github.io/media-kraken/";
const PHONE = {
...
async function snap(page, suffix, tags) {
  const name = `media-kraken-${suffix}.webp`;
...
// 1) Onboarding ("Where do you want to store your data?").
await snap(page, 1, ["Onboarding"]);

// Choose Solid, then the WebID/pod login form.
await page.getByRole("button", { name: /use solid/i }).first().click();
await page.waitForTimeout(1200);
await page.locator('input[placeholder^="https"], input[type="url"], input').first().fill(POD.idp);
await snap(page, 2, ["Login"]);

// Submit -> pod.mpeters.dev. Fill CSS credential page + consent.
await page.getByRole("button", { name: /log in with solid/i }).first().click();
...
// Back at Media Kraken: wait for the logged-in collection/home.
await page.waitForURL(/media-kraken/, { timeout: 30000 });
await page.waitForTimeout(9000); // sync + render
...
console.log(`Added ${frames.length} frames for Media Kraken: ${frames.map((f) => f.tags).flat().join(", ")}`);
